...
import { CLAIM_BULK_COLUMNS as BULK_UPLOAD_COLUMNS, CLAIM_BULK_ALLOWED_VALUES as BULK_UPLOAD_ALLOWED_VALUES } from "@/constants/claimBulkColumns";
...
const ClaimBulkUploadModal: React.FC<BulkUploadModalProps> = ({
  onClose,
  onSuccess,
  submitEndpoint = "/api/admin/claims-bulk",
}) => {
  ...
  const downloadTemplate = () => {
    ...
    if (templateFormat === "csv") {
      ...
      saveAs(new Blob([csv], { type: "text/csv;charset=utf-8;" }), "Claims_Bulk_Upload_Template.csv");
    } else {
      ...
      saveAs(
        ...
        "Claims_Bulk_Upload_Template.xlsx"
      );
    }
  };
  ...
  const handleUploadAndProcess = async () => {
    if (!file) {
      setFormError("Please choose a Claims file to upload");
      return;
    }
    setProcessing(true);
    setFormError("");
    ...
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <div className={styles.header}>
          <h3 className={styles.title}>Bulk Upload Claims</h3>
            ...
            <>
              <div className={styles.section}>
                ...
                <p className={styles.sectionHint}>
                  Download the Claims bulk upload template to see the columns (required ones are marked in the Allowed Values sheet). The Excel
                  version includes an "Allowed Values" sheet listing the exact values accepted for
                  each field (Claim Type, Reported Channel, Claim Status, dates, etc.) — rows with values
              ...
              <div className={styles.section}>
                ...
                >
                  ...
                  <span className={styles.dropzoneTitle}>
                    {file ? file.name : "Drag and drop your Claims file here"}
                  </span>
              ...
              {result && (
                <div className={styles.resultBanner}>
                  <p>
                    <strong>{result.inserted}</strong> claim{result.inserted === 1 ? "" : "s"} uploaded
                    successfully.
...
export default ClaimBulkUploadModal;
