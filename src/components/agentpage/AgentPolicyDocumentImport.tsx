      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.formGrid}>
            ...
            {text("idv", "IDV", "number")}
            {text("totalOwnDamagePremium", "Total Own Damage Premium", "number")}
...
export default function AgentPolicyDocumentImport({ onBack }: { onBack: () => void }) {
  ...
  const handleDeleteSelected = async () => {
    ...
    // Defense in depth — the checkbox itself is already disabled for
    // non-deletable rows, but never bulk-delete one that slips through.
    const ids = Array.from(selectedIds).filter((id) => {
      const row = items.find((i) => i._id === id);
      return row ? canDelete(row) : true;
    });
  ...
  const toggleSelectRow = (row: ImportRow) => {
    if (!canDelete(row)) return;
    setSelectedIds((prev) => {
      const next = new Set(prev);
  ...
  const selectableItems = items.filter(canDelete);

  const toggleSelectAll = () => {
    setSelectedIds((prev) =>
      selectableItems.length > 0 && prev.size === selectableItems.length
        ? new Set()
        : new Set(selectableItems.map((i) => i._id))
    );
  };
