...
export default function TwoWheeler() {
  ...
  const loadVehicleFromVahan = async (rc: any) => {
    try {
      setLoading(true);
      localStorage.setItem("vahanExtra", JSON.stringify(rc));

      setEngineNumber(rc.engine || "");
      setChassisNumber(rc.chassis || "");
      setSelectedYear(rc.year?.split("/")[1] || "");

      const makeName = (rc.brand || "").toUpperCase();
      ...
      console.log("MODEL LIST FOR", makeName, ">>>", modelList);

      if (!modelList.length) {
        // make didn't match Zuno's catalog — let user pick manually
        console.log("NO MODELS RETURNED for make:", makeName);
        setStep("makes");
        return;
      }
      ...
      const rcModel = String(rc.model || "")
        ...
        .replace(/\+/g, "PLUS")
        .replace(/XTEC/g, "")
        .replace(/HERO HONDA/g, "") // strip old brand prefix
        .replace(/HERO/g, "")
        .replace(/HONDA/g, "")
        .replace(/\s+/g, " ")
        .trim();
      ...
      if (!matchedModel) {
        console.log("NO MODEL MATCH for:", rcModel, "-> manual pick");
        setStep("models");
        return;
      }
      ...
      const variantRes = await fetch(
        `/api/sbi/2w/master/variant?make=${encodeURIComponent(
          makeName
        )}&model=${encodeURIComponent(matchedModel.model)}`
      );
      const variantJson = await variantRes.json();
      const variantList = Array.isArray(variantJson)
        ? variantJson
        : variantJson.data || [];
      setVariants(variantList);

      if (!variantList.length) {
        console.log("NO VARIANTS -> manual pick");
        setStep("variants");
        return;
      }
      ...
      const matchedVariant =
        variantList.find((x: any) =>
          String(x.variant)
            .toUpperCase()
            .includes(String(rc.model).toUpperCase())
        ) || variantList[0];
      ...
      // RTO
      const rtoRes = await fetch(`/api/sbi/2w/master/rto?idvCity=MUMBAI`);
      const rtoJson = await rtoRes.json();
      if (Array.isArray(rtoJson) && rtoJson[0]) {
        localStorage.setItem("selectedRto", JSON.stringify(rtoJson[0]));
      }

      setStep("vehicleDetails");
    } catch (e) {
      console.log("LOAD VEHICLE ERROR >>>", e);
      setStep("makes"); // fall back to manual instead of hanging
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.rightSection}>
              ...
              <div className={styles.yearGrid}>
                <div
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem("isNewBike", "true");
                    setStep("makes");
                  }}
                >
                  Brand New Bike ›
                ...
                {years.map((year) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem("isNewBike", "false");
                      setStep("makes");
                    }}
                  >
                    {year}
          ...
          {/* MAKE */}
          {step === "makes" && (
            <div data-aos="fade-left" className={styles.makesWrapper}>
              <button
                  ...
                  <div
                    ...
                    onClick={async () => {
                      ...
                      if (Array.isArray(data)) {
                        ...
                        console.log("MODEL API ERROR", data);
                        setModels([]);
                      }
          ...
          {/* MODELS */}
          {step === "models" && (
            <div data-aos="fade-left" className={styles.modelsWrapper}>
              <button
                className={styles.backButton}
                ...
                <div className={styles.modalOverlay}>
                  <div className={styles.modelPopup}>
                    <div className={styles.popupHeader}>
                      <h3>Other Models</h3>
                      <button onClick={() => setShowOtherModels(false)}>
                        ×
                      </button>
                    </div>
                ...
                {variants.map((item: any, index: number) => (
                  <div
                    ...
                    onClick={async () => {
                      ...
                      let idvCity = "AHMEDABAD";
                      if (vehicleNo.startsWith("MH")) idvCity = "MUMBAI";
                      else if (vehicleNo.startsWith("DL")) idvCity = "DELHI";
                      else if (vehicleNo.startsWith("GJ")) idvCity = "AHMEDABAD";

                      const res = await fetch(
                        `/api/sbi/2w/master/idv?make=${selectedMake}&model=${selectedModel}&variant=${item.variant}&idvCity=${idvCity}`
                      );
                      const result = await res.json();
                      console.log("IDV DATA", result);
                      setIdvData(result.data);
                      ...
                      console.log("RTO DATA", rtoResult);

                      const selectedRto = Array.isArray(rtoResult)
                        ? rtoResult[0]
                        : rtoResult?.data?.[0];
                      ...
                      const isNew = localStorage.getItem("isNewBike");
                      ...
                      if (isNew === "true") {
                        const vehicleData = {
                          ...
                          registrationNumber: "",
                          idv: result.data?.idvAmount?.upto1Year || "",
                        ...
                        console.log("FINAL BIKE DATA", vehicleData);
                        localStorage.setItem(
                          "selectedBikeData",
